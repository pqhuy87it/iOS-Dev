...
open class SearchTextField: UITextField {
    ...
    // MARK: - Safe Helpers for Deprecated APIs
    
    fileprivate var safeScreenHeight: CGFloat {
    ...
    fileprivate var safeKeyWindow: UIWindow? {
        if #available(iOS 13.0, *) {
        ...
        if let tableView = tableView {
            ...
            if self.direction == .down {
                ...
                if keyboardIsShowing, let keyboardHeight = keyboardFrame?.size.height {
                    // SỬA Ở ĐÂY
                    tableHeight = min((tableView.contentSize.height), (safeScreenHeight - frame.origin.y - frame.height - keyboardHeight))
                } else {
                    // SỬA Ở ĐÂY
                    tableHeight = min((tableView.contentSize.height), (safeScreenHeight - frame.origin.y - frame.height))
                }
                ...
                let tableHeight = min((tableView.contentSize.height), (safeScreenHeight - frame.origin.y - theme.cellHeight))
                UIView.animate(withDuration: 0.2, animations: { [weak self] in
    ...
    fileprivate func prepareDrawTableResult() {
        guard let frame = self.superview?.convert(self.frame, to: safeKeyWindow) else { return }
        ...
        if let keyboardFrame = keyboardFrame {
            ...
            guard let key = safeKeyWindow else { return }
